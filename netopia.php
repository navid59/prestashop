...
//        return $output; // Without Form
    ...
    protected function renderForm()
    {
        ...
        return $helper->generateForm(array($this->getConfigForm(), $this->getAgreementsForm()));
    }
    ...
    protected function getConfigFormValues()
    {
        return array(
            ...
            'NETOPIA_conditions_complete_description' => Configuration::get('NETOPIA_conditions_complete_description', true),
            'NETOPIA_conditions_prices_currency' => Configuration::get('NETOPIA_conditions_prices_currency', 'contact@prestashop.com'),
            'NETOPIA_conditions_clarity_contact' => Configuration::get('NETOPIA_conditions_clarity_contact', null),
            'NETOPIA_conditions_forbidden_business' => Configuration::get('NETOPIA_conditions_forbidden_business', null),
            'NETOPIA_conditions_has_ssl' => Configuration::get('NETOPIA_conditions_has_ssl', null),
            'NETOPIA_conditions_logo_status' => Configuration::get('NETOPIA_conditions_logo_status', null),
            'NETOPIA_terms_conditions_url' => Configuration::get('NETOPIA_terms_conditions_url', null),
            'NETOPIA_privacy_policy_url' => Configuration::get('NETOPIA_privacy_policy_url', null),
            'NETOPIA_delivery_policy_url' => Configuration::get('NETOPIA_delivery_policy_url', null),
            'NETOPIA_return_cancel_policy_url' => Configuration::get('NETOPIA_return_cancel_policy_url', null),
            'NETOPIA_gdpr_policy_url' => Configuration::get('NETOPIA_gdpr_policy_url', null),
            'NETOPIA_image_netopia_logo_link' => Configuration::get('NETOPIA_image_netopia_logo_link', null),
        );
    }
    ...
    /**
     * Create the structure of Agreement form.
     */
    protected function getAgreementsForm()
    {
        $selectBoxOptions = array(
            array(
                'id_option' => 0,
                'name' => 'No, is not appear in site at all!'
              ),
            array(
              'id_option' => 1,
              'name' => 'Yes, is appear in footer.'
            ),
            array(
                'id_option' => 2,
                'name' => 'Yes, is appear but not in footer.'
              ), 
          );
        return array(
            'form' => array(
                'legend' => array(
                    'title' => $this->l('Agreements'),
                    'icon' => 'icon-bullseye',
                ),
                'input' => array(
                    array(
                        'type' => 'switch',
                        'label' => $this->l('Declare, have complete description'),
                        'name' => 'NETOPIA_conditions_complete_description',
                        'is_bool' => true,
                        'desc' => $this->l('Declare that there is a clear and complete description of the goods and services that we will sell'),
                        'values' => array(
                            array(
                                'id' => 'active_on',
                                'value' => true,
                                'label' => $this->l('Yes, Accept')
                            ),
                            array(
                                'id' => 'active_off',
                                'value' => false,
                                'label' => $this->l('No, Not accept')
                            )
                        ),
                    ),
                    array(
                        'type' => 'switch',
                        'label' => $this->l('Declare, have clearly prices and currency'),
                        'name' => 'NETOPIA_conditions_prices_currency',
                        'is_bool' => true,
                        'desc' => $this->l('Declare that the prices and currency are clear displayed for the goods / services'),
                        'values' => array(
                            array(
                                'id' => 'active_on',
                                'value' => true,
                    ...
                    array(
                        'type' => 'switch',
                        'label' => $this->l('Declare, clarity of contact info'),
                        'name' => 'NETOPIA_conditions_clarity_contact',
                        'is_bool' => true,
                        'desc' => $this->l('Declare that, the contact details of the company (SC, CUI, address, telephone, fax / e-mail,...) are clearly displayed on the website'),
                        'values' => array(
                            array(
                                'id' => 'active_on',
                                'value' => true,
                    ...
                    array(
                        'type' => 'switch',
                        'label' => $this->l('Declare, of business / activities'),
                        'name' => 'NETOPIA_conditions_forbidden_business',
                        'is_bool' => true,
                        'desc' => $this->l('Declare that we are not do trade on the list above also in none of the forbidden business / services'),
                        'values' => array(
                            array(
                                'id' => 'active_on',
                                'value' => true,
                    ...
                    array(
                        'type' => 'switch',
                        'label' => $this->l('Declare, to use SSL Certificate valid'),
                        'name' => 'NETOPIA_conditions_has_ssl',
                        'is_bool' => true,
                        'desc' => $this->l('If your website has a valid SSL certificate'),
                        'values' => array(
                            array(
                                'id' => 'active_on',
                                'value' => true,
                                'label' => $this->l('Has SSL Certificate')
                            ),
                            array(
                                'id' => 'active_off',
                                'value' => false,
                                'label' => $this->l('Has not SSL Certificate')
                            )
                        ),
                    ),
                    array(
                        'type' => 'select',
                        'label' => $this->l('Declare, mandatory logo is visible in your site'),
                        'name' => 'NETOPIA_conditions_logo_status',
                        'required' => true,
                        'desc' => $this->l('If the Visa card, Master Card and NETOPIA Payments logo is appearing in site footer.'),
                        'options' => array(
                            'query' => $selectBoxOptions,                  // $options contains the data itself.
                    ...
                    array(
                        ...
                        'desc' => $this->l('ex. somewhere/privacy_policy OR privacy_policy.html | without your actual domain name.'),
                        'name' => 'NETOPIA_terms_conditions_url',
                    ...
                    array(
                        ...
                        'prefix' => '<i class="icon icon-link"></i>',
                        'desc' => $this->l('ex. somewhere/privacy_policy OR privacy_policy.html | without your actual domain name.'),
                        'name' => 'NETOPIA_privacy_policy_url',
                    ...
                    array(
                        ...
                        'type' => 'text',
                        'prefix' => '<i class="icon icon-link"></i>',
                        'desc' => $this->l('ex. somewhere/delivery_policy OR ex. delivery_policy.html | without your actual domain name.'),
                        'name' => 'NETOPIA_delivery_policy_url',
                        'label' => $this->l('Delivery policy'),
                        'required' => true,
                    ),
                    array(
                        'col' => 6,
                        'type' => 'text',
                        'prefix' => '<i class="icon icon-link"></i>',
                        'desc' => $this->l('ex. somewhere/return_cancel OR ex. return_cancel.html | without your actual domain name.'),
                        'name' => 'NETOPIA_return_cancel_policy_url',
                        'label' => $this->l('Return / Cancel policy'),
                        'required' => true,
                    ),
                    array(
                        'col' => 6,
                        'type' => 'text',
                        'prefix' => '<i class="icon icon-link"></i>',
                        'desc' => $this->l('ex. somewhere/gdpr_policy OR ex. gdpr_policy.html | without your actual domain name.'),
                        'name' => 'NETOPIA_gdpr_policy_url',
                        'label' => $this->l('GDPR policy'),
                        'required' => true,
                    ),
                    array(
                        'col' => 6,
                        'type' => 'text',
                        'prefix' => '<i class="icon icon-picture"></i>',
                        'desc' => $this->l('ex. http://www.yoursite.com/images/netopia-patements.svg | Full URL of the image.see the following url to see how the logo should be look like.https://netopia-payments.com/brand/ '),
                        'name' => 'NETOPIA_image_netopia_logo_link',
                        'label' => $this->l('NETOPIA Payments logo'),
                        'required' => true,
                    ),
                ),
                'submit' => array(
...
//            $this->context->controller->addJquery();
    ...
    public function hookActionPaymentCCAdd()
    {
         ...
         * */
        $this->paymentUrl = (Configuration::get('NETOPIA_LIVE_MODE')) ? 'https://secure.mobilpay.ro/' : 'http://sandboxsecure.mobilpay.ro/';
        $x509FilePath 	= dirname(__FILE__) .'/certificates/sandbox.YN8Q-RH4J-39C1-FPAG-2P8A.public.cer';
